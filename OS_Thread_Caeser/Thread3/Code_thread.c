...
int main(void){
	...
	for(i=0;i<10;i++){
		...
		execution();
		te=clock();
		double tot=(double)(te-ts)/(double)clk;
		moyenne += tot;
		printf("Temps du test (s) : %lf\n",tot);
	}
	printf("Temps moyen du test (s) : %lf\n", moyenne/10);
	data[0]=moyenne/10;
	...
	FILE* fichier;
	char* filen = "./Data";
	fichier = fopen(filen, "w");
	int a;
	for(a=0;a<6;a++){
		fprintf(fichier,"Temps moyen d'execution avec %d thread (s) : %lf\n",(a+1),data[a]);
	}
	fclose(fichier);
